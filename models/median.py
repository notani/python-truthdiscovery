...
u"""
Example:
python models/median.py population -e -v
"""


class Median(BasicModel):
    def __init__(self, dataset='population', has_header=False,
                 evaluation=False,
                 verbose=False):
        BasicModel.__init__(self, dataset=dataset, has_header=has_header,
                            evaluation=evaluation,
                            name='Median', verbose=verbose)

    def run(self):
        if self.verbose:
            self.logger.info('Running')

        g = self.reader.data.groupby('dataItemID')['value'].median()
...
def main(args):
    model = Median(has_header=args.flag_has_header,
